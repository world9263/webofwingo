...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['match_id'])) {
    ...
    // Update team names in the matches_data table
    $update_query = "UPDATE matches_data SET team1_name='$team1_name', team2_name='$team2_name' WHERE match_id='$match_id'";
    if (mysqli_query($conn, $update_query)) {
        //echo "Team names for Match ID: $match_id updated successfully!";
    } else {
        echo "Error updating team names: " . mysqli_error($conn);
    }
}

// Query to fetch data from matches_data table
$query = "SELECT match_id, team1_name, team2_name, bet_end_time FROM matches_data";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        // Build HTML table rows with fetched data
        $dataRow .= "<tr>";
        $dataRow .= "<td>" . $row["match_id"] . "</td>";
        $dataRow .= "<td>" . $row["team1_name"] . "</td>";
        $dataRow .= "<td>" . $row["team2_name"] . "</td>";
        $dataRow .= "<td>" . $row["bet_end_time"] . "</td>";
        $dataRow .= "<td>";
        $dataRow .= "<form method='post' action='" . htmlspecialchars($_SERVER["PHP_SELF"]) . "'>";
        $dataRow .= "<input type='hidden' name='match_id' value='" . $row["match_id"] . "'>";
        $dataRow .= "<input type='text' name='team1_name' value='" . $row["team1_name"] . "' required>";
        $dataRow .= "<input type='text' name='team2_name' value='" . $row["team2_name"] . "' required>";
        $dataRow .= "<input type='submit' value='Update'>";
        $dataRow .= "</form>";
        $dataRow .= "</td>";
        $dataRow .= "</tr>";
    }
} else {
    $dataRow = "<tr><td colspan='5'>No matches found</td></tr>";
}
        ...
        <div class="content">
            ...
            <nav class="navbar navbar-expand bg-light navbar-light sticky-top px-4 py-0" style="background-color: #071251 !important;">
                ...
                <div class="navbar-nav align-items-center ms-auto">
                    <!-- Add Create Match button here -->
                    <button class="btn btn-primary me-2" onclick="location.href='add_match.php';">
                        Create Match
                    </button>
