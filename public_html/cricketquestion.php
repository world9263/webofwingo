...
// Function to update answers or correct answers
function updateField($conn, $table, $id, $field, $value)
{
    $query = "UPDATE $table SET $field = '$value' WHERE question_id = $id";
    if (mysqli_query($conn, $query)) {
        // Insert into cricket_bets table
        $insert_query = "INSERT INTO cricket_bets (answer_correct) VALUES ('$value')";
...
// Check if form is submitted for updating fields or deleting question
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['answer']) && isset($_POST['question_id'])) {
        $answer = $_POST['answer'];
        $question_id = $_POST['question_id'];
        if (updateField($conn, 'cricket_questions', $question_id, 'answers', $answer)) {
            echo "Answer updated successfully.";
        } else {
            echo "Error updating answer.";
        }
    } elseif (isset($_POST['correct_answer']) && isset($_POST['question_id'])) {
        $correct_answer = $_POST['correct_answer'];
        $question_id = $_POST['question_id'];
        if (updateField($conn, 'cricket_questions', $question_id, 'correct_answer', $correct_answer)) {
            echo "Correct answer updated successfully.";
        } else {
            echo "Error updating correct answer.";
        }
    } elseif (isset($_POST['delete']) && isset($_POST['question_id'])) {
        $question_id = $_POST['question_id'];
        $query = "DELETE FROM cricket_questions WHERE question_id = $question_id";
        if (mysqli_query($conn, $query)) {
            echo "Question deleted successfully.";
        } else {
            echo "Error deleting question.";
        }
    } elseif (isset($_POST['new_question']) && isset($_POST['new_answers']) && isset($_POST['match_id'])) {
        $new_question = $_POST['new_question'];
        $new_answers = $_POST['new_answers'];
        $match_id = $_POST['match_id'];
        $query = "INSERT INTO cricket_questions (question, answers, match_id) VALUES ('$new_question', '$new_answers', '$match_id')";
        if (mysqli_query($conn, $query)) {
...
// Fetch data from cricket_questions table in descending order based on creation date
$query = "SELECT *, DATE(created_at) as creation_date FROM cricket_questions ORDER BY created_at DESC";
...
while ($row = mysqli_fetch_array($result)) {
    $dataRow .= "<tr>";
    $dataRow .= "<td>" . $row['question_id'] . "</td>";
    $dataRow .= "<td>" . $row['question'] . "</td>";
    $dataRow .= "<td><form method='post'><textarea name='answer' rows='4' cols='20'>" . $row['answers'] . "</textarea><input type='hidden' name='question_id' value='" . $row['question_id'] . "'><br><button type='submit'>Update</button></form></td>";
    $dataRow .= "<td><form method='post'><textarea name='correct_answer' rows='4' cols='5'>" . $row['correct_answer'] . "</textarea><input type='hidden' name='question_id' value='" . $row['question_id'] . "'><br><button type='submit'>Update</button></form></td>";
    $dataRow .= "<td>" . $row['bet_amount'] . "</td>";
    $dataRow .= "<td>" . $row['username'] . "</td>";
    $dataRow .= "<td>" . $row['winning_amount'] . "</td>";
    $dataRow .= "<td>" . $row['creation_date'] . "</td>";
    // Add form for deleting a question
    $dataRow .= "<td><form method='post'><input type='hidden' name='question_id' value='" . $row['question_id'] . "'><button type='submit' name='delete'>Delete</button></form></td>";
    $dataRow .= "</tr>";
}


// Fetch matches from matches_data table where bet end time is greater than current time
$current_time = date('Y-m-d H:i:s');
$matches_query = "SELECT match_id, match_details FROM matches_data WHERE bet_end_time > '$current_time'";
$matches_result = mysqli_query($conn, $matches_query);
$matches_options = "";
while ($match_row = mysqli_fetch_array($matches_result)) {
    $matches_options .= "<option value='" . $match_row['match_id'] . "'>" . $match_row['match_details'] . "</option>";
}
  ...
  <table style="background-color: White;">
       <tr>
        ...
        <form method="post">
            ...
            <label for="new_answers">Answers:</label><br>
            <input type="text" id="new_answers" name="new_answers" required><br>
            <label for="match_id">Select Match:</label><br>
            <select id="match_id" name="match_id" required>
                <option value="">Select Match</option>
                <?php echo $matches_options; ?>
            </select><br><br>
